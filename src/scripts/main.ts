...
export default function dimension(inputsValue: IFormValues) {
  ...
  const { b, h, d, fck, fyk, E, qsic, qsis, qsif, serviceBendingMoment, beta } = inputsValue

  const { alamb, alfac, eu, qlim } = typeConcreteVariables(fck as number, beta as number)
